...
export default class MyTasksTable extends Component {
  ...
  formatDate(data) {
    return <Moment format="DD-MM-YYYY  hh:mm">{data}</Moment>;
  }
  ...
  itemViewFormatter = (props) => {
    ...
    return (
      <Link
        ...
        to={`/po-request/Taskview/${props.value}/${props.dependentValues.Id}`}>
        <Button size="sm" color="success" id="RWButton">
          Click here
        </Button>{" "}
      </Link>
    );
  ...
  //       prefix={`${JSON.parse(props.dependentValues.POJSONData).Currency} `}
  //     />
  //   );
  // };
  DateFormatter = (props) => (
  ...
  PONumber = (props) => {
    let temp = props.value.split("@split");
    let POData = JSON.parse(temp[1]);
    console.log(POData);
    return (
      <div>
  ...
  GrandTotalFormatter = (props) => {
    ...
    return (
      <NumberFormat
        value={POData.GrandTotal || 0}
        displayType={"text"}
        thousandSeparator={true}
        prefix={POData.Currency + " "}
      />
      // POData.Currency + " " + POData.GrandTotal
    );
  };
  CheckPriority = (props) => {
    if (props.value === "High") {
  ...
  initColumnDefs = () => {
    this._columns = [
...

      {
        ...
        name: "PO Number",
        width: "15%",
        formatter: this.PONumber,
      },
      {
        key: "Body",
        name: "Department",
        width: "15%",
        formatter: this.Department,
      },
      {
        key: "Body",
        name: "Grand Total",
      ...
      <>
        <Card style={tableContainer}>
          <Row>
            ...
            <Col md={12}>{TaskDetailsTable}</Col>
          </Row>
        </Card>
      </>
    );
